...
# solution
global_reef_fish_biomass <- read_csv(file = "./data/global_reef_fish_biomass.csv")
global_reef_fish_abundance <- read_csv(file = "./data/global_reef_fish_abundance.csv")
survey_metadata <- read_delim(file = "./data/survey_metadata.csv",
                              delim = ";",
                              skip = 70)
```
...
-   Syntaxe claire et cohérente
    -   "lien" avec le SQL
    -   deux "familles" de fonctions : résumer et vectorisation <br/> <br/> <br/> ![](images/summary_function.png){.absolute top="320" left="15"} ![](images/vectorized_function.png){.absolute top="320" right="15"}
...
-   A l'aide d'une fonction dplyr, verifier s'il y a des données dupliquées dans le jeu de données "global_reef_fish_biomass"
-   En une seul fois, selectionner les données dont la variable total est différent de 0, ou la famille de l'espèce est différente de "Mullidae" ou "Apogonidae" et pour finir ou la classe de taille associé est \<= 10.
...
-   Crée une variable "survey_id_full" qui est la concaténation des variables survey_id, species_name et size_class
...
-   Fonctions qui vont créer une nouvelle table avec des resumés statistique. De manière globale, les fonctions associées vont utiliser en entrée des vecteurs et en sortie une ou plusieurs valeurs (inférieur en nombre au nombre de vecteurs d'entrée) <br/> <br/> <br/> ![](images/summary_function.png){.absolute top="200" left="250" width="50%"}
...
-   Pour le moment on à fait des opérations sur l'ensemble du jeu de données, pas très pertinent non ?
...
-   Attention à ce que vous faite ! Par défaut, les groupes sont gardés en mémoire dans votre jeu de données de sortie (lisez les messages de R). Pour y remédier :

```{r echo=TRUE, eval=FALSE}
# via un summarise (argument experimental, attention à la pérénité)
.groups
# de manière plus générale
ungroups()
```

-   Cette notion de groupe peut être utilisé avec d'autres fonctions (comme un mutate)

## Exercice 2.3 {.smaller auto-animate="true"}

-   À partir du jeu de données de sortie de l'exercice 2
-   Effectuer la somme de la biomasse (variable biomass) en fonction de la variable survey_id_full crée précédemment
-   Pour survey_id_full répétés, ne garder que la première ligne de chaque répétition. Attention à bien conserver toutes les colonnes associées (une fois la fonction "trouvé" regarder du côté des arguments en option)
-   Créer un nouveau jeu de données qui exprime le nombre d'échantillons disponible par famille et vérifier la cohérence du nombre total obtenu avec le jeu de données de départ.
...
### case_when : généralisation vectorisé de if(s)

-   Vectorisation de multiple if_else sous R et CASE WHEN en SQL
...
-   Attention, aucune verification de cohérence des jointures
...
### Jointures de type relationnelle
...
-   Attention à bien spécifier l'argument by (ne pas laisser place à l'interprétation !)
    -   On peut spécifier un seul ou plusieurs colonnes si les noms sont identiques entre les deux jeux de données
    -   Sinon on spécifier le nom du jeu de données à "gauche" et celui qui est à "droite"
:::
...
-   Faire la "meilleure" jointure avec le jeu de données survey_metadata et m'expliquer votre choix
-   Pour vous aider :
...
<center>La valeur 11.21 n'apparait qu'une seule fois, ce sont bien les combinaisons uniques.<\center>
:::
...
-   Créez une nouvelle variables "pk" qui est l'union entre le survey_id et sp_size avec comme séparateur un "\_". Pensez à enchainer les opérations
...
-   Transformez les en format tidy. Les nouvelles colonnes seront "sp_size" et "abundance"
-   Créez une nouvelle variables "survey_id_full" à partir des colonnes survey_id et sp_size avec comme séparateur un "\_". Pensez à enchainer les opérations
...
On utilise la fonction `str_extract()` et ces arguments principaux :
...
::: {.column width="50%"}
```{r results='hide'}
str_extract(string  = tmp$ville, pattern = "Montpel")
```

```{r echo = FALSE}
cat(str_detect(string  = tmp$ville, 
               pattern = "Montpel"), sep = "\n")
```
:::
:::
...
::: {.column width="50%"}
```{r results='hide'}
# Les noms à avant le replacement
tmp$nom
...
**Question 3** : A partir du fichier global_reef_fish_biomass.csv chargé avec `read_delim()`, enchaîner le pipe pour obtenir un data.frame résumant le nombre d'observation dans chacune des classes, pour le site n°2000951 seulement.
...
```{r}
load("./data/biomass_survey_abundance_exercice3_6.RData")
survey_med <- biomass_survey_abundance_exercice3_6 %>%
  filter(str_detect(location, "Mediterranean") == TRUE)

kable(survey_med[1:5,], html_font = "Computer Modern", table.attr = "style='width:60%;'")
```
...
kable(survey_med[1:3,], html_font = "Computer Modern", table.attr = "style='width:50%;'")

save(survey_med, file = "./data/survey_med.RData")
```
...
```{r echo=FALSE}
tmp <- global_reef_fish_biomass %>%
  filter(survey_id == "2000951") %>%
  group_by(class) %>%
  summarise(n_class = n())
kable(tmp, html_font = "Computer Modern", table.attr = "style='width:50%;'")
```

## Exercice 4.1.1 - Correction Q4 {auto-animate="true"}

**Question 4** : Résolvez le problème de façon à obtenir une tibble de dimension 2x2.
...
-   Le plus difficle est de formuler de la meilleur façon ce que vous voulez
    -   Il y a souvent plusieurs façon d'arriver à ce que vous voulez
...
dmy()
ymd_hms()
# Beaucoup de possibilité, jeter un coup d'oeil sur le cheatsheet
```
...
-   Les fuseaux horaires, un cauchemar par le passé, maintenant ça va mieux
    ...
    -   Deux fonctions clées

```{r echo=TRUE, eval=FALSE}
# Pour exprimer un élément dans un autre fuseau horaire
with_tz()
# Pour forcer le changement d'un fuseau horaire mais on concervant la même heure
force_tz()
```
...
-   3 classes pour calculer des intervalles de temps
    ...
    -   Le plus "dure" est de définir votre date-temps, après c'est une syntaxe mathématique

## Bonus

-   Les fonctions sont vectorisables !

-   Sur certains jeux de données, les dates-temps sont stocké en seconde depuis 1970-01-01 00:00:00 UTC. Utiliser cette fonction dans ce cas la :

```{r echo=TRUE, eval=FALSE}
as_datetime()
```

-   Possibilité de faire des arrondies

```{r echo=TRUE, eval=FALSE}
# arrondie inférieur sur "l'unité" la plus proche
floor_date()
# arrondie inférieur sur "l'unité" la plus proche
round_date()
...
3)  C'est un enchainement plus ou moins long d'opérations.
    ...
    ![](images/bogdanoff1.jpg){.absolute top="500" left="100" width="21%"}
    ![](images/bogdanoff1.5.jpg){.absolute top="500" left="300" width="25%"}
    ![](images/bogdanoff2.jpg){.absolute top="500" left="700" width="25%"}
:::
:::

## Petit apperçu des possibilités :

<center>[Le site **From Data To Viz**, une pépite](https://r-graph-gallery.com/){preview-link="true"}
...
Pour la structure du la figure, la fonction é toujours utiliser est `ggplot()` . Les arguments principaux sont :
...
Quel est le pays dans lequel on retrouve le plus de biomasse par transect. Formatter le jeu de données issues de l'exercice 3.6 et représenter visuellement les données sous forme de boites à moustache, **seulement pour les données prélevées en 2011**.
...
Ce ggplot n'est cependant pas entièrement satisfaisant ... <br> 
Nous avons du mal à savoir visuellemenent quelle est la 2nd ou la 3ème nation avec le plus de biomasse.

. . .

Une solution est d'uiliser la fonction `reorder()` pour ordonner l'axe des x. 

Utiliser cette fonction pour ordonner l'axe des x de façon décroissance selon la valeur **médiane**. Nommer le plot **boxplot_biomass_country**. <br> N'hésitez pas à regarder l'aide `?reorder` ou à chercher sur internet.
...
```{r fig.align="center"}
# Reoder en fonction de la valeur médiane 
...
Donnons une couleur par sexe. Comme c'est liée à l'esthétisme de la figure, il faut retourner à l'argument `color` dans la fonction `aes()`.
...
Renomer proprement les axes de boxplot_biomass_country.
...
La fonction `theme()` permet de lifter BEAUCOUP de choses (`?theme`). <br>
...
Dans la fonction `element_text()`, un argument permet de tourner les légendes des axes pour que les pays soient plus lisible. Trouver cet argument et appliquer une rotation de 45° à la légende de l'axe x. Choissir votre arrière plan préféré.
...
Dans la fonction `element_text()`, un argument permet de tourner les légendes des axes pour qu'ils soient plus lisible. Trouver cet argument et appliquer une rotation de 45° à la légende de l'axe x. Choissir votre arrière plan préféré.
...
3.  Reconstruire un boxplot en attribuant un couleur pour chaque continent.
:::
...
<center> On ne peut pas donner une seule couleur à une variable continue !
...
A partir des données data_biomass_5country, construire un diagramme en barres en de la biomasse moyenne par transect.
Classer l'axe des x de la plus dans l'ordre **croissant**. 
Trouvez la façon de rendre les barres moins larges et de changer leur couleur.
...
Toutes les fonctions et la syntaxe montraient précédemment fonctionnent ici
...
```{r echo=TRUE, eval=FALSE}
# Pour les objets spatialisés de type vectoriels au format sf
geom_sf()
...
-   Applications
    ...
    -   Afficher sur chaque pays correspond un rond dont la taille est proportionnelle à la population
...
-   Construisez une carte des transects en Méditerranée avec une couleur par "site_name" et un shape par "method". Choisissez une palette de couleur cohérente avec les données et masquez la légende
...
-   Recentrez la carte autour des pays d'intérêts
...
-   Customisez la carte avec la flêche du Nord, le nom des axes et un titre. Renommez la légende et ajouter une palette de couleur distinguant terre et mer pour la bathymétrie
