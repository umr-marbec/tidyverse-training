...
-   Partie 2 : Manipulation de données avec dplyr
    ...
    -   2.2 : Transformation des données avec dplyr
...
# solution
# A revoir
global_reef_fish_abundance_biomass_final <- mutate(.data = tmp1,
...
-   À partir du jeu de données de sortie de l'exercice précédent
-   Calculer la biomasse moyenne d'un invididu pour chaque échantillon
-   Exprimer cette nouvelle variable dans un jeu données avec la famille et le nom de l'espèce et sa famille (variables family et species_name)
-   Calculer dans une nouvelle variable, la biomasse moyenne des individus par famille.
-   Créer une nouveau jeu de données qui exprime le nombre d'échantillons disponible par famille et verifier la cohérence du nombre total obtenu avec le jeu de données de départ.
...
tmp4 <- tmp3 %>%
  mutate(mean_biomass_sample = biomass / total) %>%
  dplyr::select(family,
                species_name,
                mean_biomass_sample) %>%
  dplyr::group_by(family) %>%
  summarise(mean_biomass_sample_family = mean(mean_biomass_sample,
                                              na.rm = TRUE),
            .groups = "drop")
