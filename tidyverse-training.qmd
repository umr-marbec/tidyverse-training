...
global_reef_fish_biomass <- read_delim(file = "./data/Global_reef_fish_biomass.csv", delim = ",")
...
fish1 <- read.delim("./data/Global_reef_fish_biomass.csv", 
                   sep = ",")
fin <- Sys.time()
print(paste("L'import a duré : ", fin-debut, "secondes"))
```
...
-   Transformez les en format tidy. Les nouvelles colonnes seront "sp_size" et "abundance"
-   Créez une nouvelle variables "pk" à partir des colonnes survey_id et sp_size avec comme séparateur un "\_". Pensez à enchainer les opérations
...
abundance_long <- abundance %>%
  ...
  unite(.,col="pk",c(survey_id,sp_size),sep="_")
```
...
La fonction `str_detect()` peut donc être utilisée avec `filter()` pour faire une <br> sélection des lignes qui nous intéresse dans le tableau.\<\center\>
...
```{r echo = TRUE, fig.align='center', out.width="65%"}
couche.3 <- couche.2 +
  geom_smooth(color = "red", method = "lm") + # le modèle linéaire 
...
Quel est le pays dans lequel on retrouve le plus de biomasse par transect. Formatter le jeu de données issues de l'exercice 3.6 et représenter visuellement les données sous forme de boites à moustache.
...
Ce ggplot n'est cependant pas entièrement satisfaisant ... <br> Nous avons du mal à savoir visuellemenent quelle est la 2nd ou 3ème nation avec le <br> plus de biomasse.

. . .

Une solution est d'uiliser la fonction `reorder()` pour ordonner l'axe des x. N'hésitez pas à regarder l'aire `éreorder` ou à chercher sur internet.
...
Utiliser cette fonction pour ordonner l'axe des x de façon décroissance selon la valeur **médiane**. Nommer le plot **boxplot_biomass_country**. <br> N'hésitez pas à regarder l'aide `éreorder` ou à chercher sur internet.
