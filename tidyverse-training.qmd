...
# Partie III : Visualisation des données avec ggplot
...
3.  Les Couleurs
    ...
    ```{r echo=FALSE, out.width="85%"}
    p3 <- p1 + ggplot2::geom_point(mapping = ggplot2::aes(color = Petal.Width), size = 5) + scale_color_viridis_c()
    p3
    ```
:::
...
1.  `aes()` dans `ggplot()`
    ...
    ```{r echo=TRUE, eval = FASLE, warnings=FALSE}
    p2 <- p1 + 
      ggplot2::geom_point()
    ```

2.  `aes()` dans `geom_point()`
...
voir https://ggplot2-book.org/scale-colour.html#colour-discrete
