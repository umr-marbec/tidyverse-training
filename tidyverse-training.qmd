...
Lire le fichier survey_metadata et le stocker dans un objet R `survey_metadata` en sélectionnant le pays Indonesia, une profondeur inférieure à 20m et une visibilité supérieure à 10m.
...
-   En une seule fois, sélectionner les données dont la variable "biomass" est différente de 0, où la famille est différente de "Mullidae" et "Apogonidae" et pour finir où la classe de taille associée est \<= 10.
...
-   À partir du jeu de données de sortie de l'exercice 2.2
...
En utilisant les fonctions vues précédemment - Quel est le pays où la biomasse totale par transect est la plus élevée ? - Formattez le jeu de données issues de l'**exercice 5.1** et représentez visuellement les données sous forme de boites à moustache, **seulement pour les données prélevées en 2011**. Nommer le jeu de données **data_biomass_country**.
