...
![](images/bogdanoff1.jpg){.absolute top="521" left="50" width="22%"}         
![](images/bogdanoff1.5.jpg){.absolute top="500" left="360" width="25%"} 
...
A partir de **biomass_survey_abundance_exercice3_6**, obtenez un data.frame de trois colonnes 
**size_class_mm**, **biom_abun**, **value** pour les données prélevées en 2011.
...
Réaliser un diagramme en barre permettant illustre le mieux les cas où l'abondance est supérieure à la biomasse
et vise-versa.
...
```{r echo=TRUE, eval=FALSE}
# Assigner le système de coordonnées
survey_med <- st_set_crs(survey_med,"+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs") 
# solution
```

## Exercice 5.9 corrigé {.smaller auto-animate="true"}

-   Construisez une carte des transects en Méditerranée avec une couleur par profondeur. Choisissez une palette de couleurs cohérente avec les données et masquez la légende

```{r, eval=FALSE, fig.align='center'}
# solution
med_map1 <- ggplot(data = survey_med) +
  geom_sf(aes(color=as.factor(depth)),show.legend=FALSE) +
...
med_map2 <- med_map1  +
  geom_sf(data=med_countries, fill="gray92")
med_map2
```
...
(xmin=-20,xmax=60,ymin=25,ymax=45)
...
## Exercice 5.12 {.smaller auto-animate="true"}
...
## Exercice 5.12 corrigé {.smaller auto-animate="true"}
...
country_med_nsample <- left_join(med_countries,survey_med_nsample,by=c("name"="country")) %>%
  drop_na(n_sample)
```
...
## Exercice 5.13 {.smaller auto-animate="true"}
...
## Exercice 5.13 corrigé {.smaller auto-animate="true"}
...
```{r, eval=FALSE, fig.align='center'}
# solution
med_map5 <- med_map4 + geom_raster(data=rasdf, aes(x=x,y=y,fill=bathy_med))
med_map5
```
...
## Exercice 5.14 {.smaller auto-animate="true"}
...
## Exercice 5.14 corrigé {.smaller auto-animate="true"}

-   Customisez la carte avec la flêche du Nord, le nom des axes et un titre. Renommez la légende et ajoutez une palette de couleurs distinguant terre et mer pour la bathymétrie

```{r echo=TRUE, eval=FALSE, fig.align='center'}
# solution
med_map6 <- ggplot(data = survey_med) +
  geom_raster(data=rasdf, aes(x=x,y=y,fill=bathy_med)) +
  geom_sf(data=med_countries, fill="gray92") +
  geom_sf(aes(color=as.factor(depth)), show.legend = FALSE) +
  scale_colour_viridis_d() +
  theme_bw() +
...
## Exercice 5.15 {.smaller auto-animate="true"}
...
## Exercice 5.15 corrigé {.smaller auto-animate="true"}

-   Sauvegardez votre belle carte :)

```{r echo=TRUE, eval=FALSE}
# solution
ggsave("RLS_med.pdf",plot=med_map6, width = 15, height = 10, units = "cm")
```
