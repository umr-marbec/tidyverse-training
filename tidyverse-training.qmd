...
```{r, echo=TRUE, eval=FALSE}
global_reef_fish_abundance <- read_delim(file = "./data/global_reef_fish_abundance.csv",
                                         delim = ",",
                                         quote = "\"")
...
-   Supprimer la première colonne du jeu de données (utiliser son indice) ainsi que la colonne reporting_name
...
biomass_survey_exercice2_4 <- biomass_exercice2_3 %>%
  right_join(survey_metadata_distinct,
             by = "survey_id")
```

# Partie 3 : Nettoyage des données avec `tidyr`

<center>![](images/logo_tidyr.png){width="20%"}<center>
...
::: {layout-ncol="3"}
![Nuage de points](images/Diagramme_empile.png){width="\"30%"}

![Boxplots](images/Diagramme_empile.png){width="30%"}

![Violin Plot](images/Diagramme_empile.png){width="30%"}
:::
...
- Ajouter la carte de bathymétrie. Pour la charger utiliser la commande raster::raster("bathy_med.tif)
Que remarquez vous ?
