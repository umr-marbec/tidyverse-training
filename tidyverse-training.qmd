...
-   Ou bien avec `read_csv` : delimiteur par défaut = virgule
...
Puis on essaie d'importer sous R ce nouveau csv au format "francais"
...
-   Option 2 : `read_delim` en précisant le séparateur décimal avec locale() pour éviter de lire les nombres comme du caractère
...
-   Afficher les spécifications avec `spec`

```{r echo=TRUE, eval=FALSE}
spec(global_reef_fish_biomass)
```

-   Simplifier les spécifications avec `col_condense`
...
Ce fichier contient des métadonnées caractérisant les campagnes (survey_id) 

Particularités : séparateur ";" et 70 lignes de commentaires au début qui ne doivent pas être lues : utilisation du paramètre `skip =`
...
## ***readr*** permet de lire depuis une archive, ce que ne permettent pas les fonctions de base
...
## ***readr*** permet de lire depuis plusieurs fichiers et même répertoires
...
## La sélection de colonnes lors de la lecture avec ***readr*** est facilitée
...
## On peut renommer certaines colonnes avec ***readr***
...
popEU <- readRDS("./data_tmp/popEU.RDS")
```
...
```{r echo=FALSE, eval=TRUE}
# solution
load("./data_tmp/biomass_survey_exercice2_4.RData")
biomass_abundance <- biomass_survey_exercice2_4 %>%
  left_join(., abundance_long, by = "survey_id_full")
...
-   Transformez le tibble en sf object avec la commande 
sf::st_as_sf(survey, coords = c("longitude", "latitude"), remove = FALSE)
...
save(biomass_survey_abundance_exercice3_6,
     file = "./data_tmp/biomass_survey_abundance_exercice3_6.RData")
```
...
## La cartographie avec ggplot2 {.smaller}

### Une petite introduction pour commencer

### Les données géographiques 

Ce sont des données ayant une localisation dans l'espace. On peut les visualiser sur une carte. Pour les répresenter il existe deux formats principaux : le vecteur et le raster.

### Un raster c'est quoi ?

Un **raster** est une image dans laquelle chaque pixel a une seule valeur et est géo-référencé.
Chaque pixel a la même taille et la résolution dépend du nombre de pixels.
Sous R, il existe une librairie spécialement développée pour traiter des fichiers raster. C'est la librairie `raster`

### Un vecteur c'est quoi ?

Un **vecteur** est composé de 2 éléments principaux:
  
-   la **table attributaire** : permet de décrire les informations qui lui sont associées. Le nombre d'informations n'est pas limité
  
-   la **partie graphique** (sa forme) : il existe trois types de géometrie pour définir les vecteurs (point, ligne, polygone)

L'ensemble de ces composantes (informations, géométrie, projection, ...) forme un objet que l'on appelle shapefile. 

La librairie `sf` permet de gérer les formats vecteur sous R. 
  
Pour plus d'informations [cliquez ici](https://naturagis.fr/cartographie-sig/difference-vecteur-raster/)  
...
-   Afficher sous format texte le code ISO3 des pays sans superposition des étiquettes (check_overlap)
...
## Exercice 5.9 {.smaller auto-animate="true"}
...
## Exercice 5.9 corrigé {.smaller auto-animate="true"}

-   Construisez une carte des transects en Méditerranée avec une couleur par profondeur. Choisissez une palette de couleurs cohérente avec les données
```{r echo=FALSE, eval=TRUE, fig.align='center'}
# solution
load("./data_tmp/survey_med.RData")
```
...
## Exercice 5.10 {.smaller auto-animate="true"}
...
## Exercice 5.10 corrigé {.smaller auto-animate="true"}
...
## Exercice 5.11 {.smaller auto-animate="true"}
...
## Exercice 5.11 corrigé {.smaller auto-animate="true"}
...
## Exercice 5.12 {auto-animate="true"}
...
## Exercice 5.12 corrigé {auto-animate="true"}
...
med_map4
```
## Exercice 5.13 {auto-animate="true"}
...
## Exercice 5.13 corrigé {auto-animate="true"}
...
```{r echo=FALSE, eval=TRUE, fig.align='center'}
# solution
bathy_med <- raster::raster("./data_tmp/bathy_med.tif")
rastdf <- as.data.frame(bathy_med, xy = TRUE)
med_map5 <-
  med_map4 + geom_raster(data = rastdf, aes(x = x, y = y, fill = bathy_med))
med_map5
```
<br>
<center>Importance dans l'ordre d'ajout des couches<center>
...
## Exercice 5.14 {auto-animate="true"}

-   Customisez la carte avec la flèche du Nord, le nom des axes et un titre. Renommez la légende et ajoutez une palette de couleurs distinguant terre et mer pour la bathymétrie

```{r echo=TRUE, eval=FALSE}
# solution
```

## Exercice 5.14 corrigé {auto-animate="true"}
...
med_map6
```
## Exercice 5.15 {auto-animate="true"}
...
## Exercice 5.15 corrigé {auto-animate="true"}
